...
public class RequestProcessorThread implements Callable<Boolean> {
    private ArrayBlockingQueue<UserService> queue;
    public RequestProcessorThread(ArrayBlockingQueue<UserService> queue) {
        this.queue = queue;
    }

    @Override
    public Boolean call() throws Exception {
        UserService request = queue.take();
        Map<Integer,Boolean> flagMap = RequestQueue.getInstance().getBooleanMap();
        //不需要强制刷新的时候，查询请求去重处理
        if (!request.isForceFefresh()){
            if (request instanceof UserServiceImpl) {//如果是更新请求，那就置为false
